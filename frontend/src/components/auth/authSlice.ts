...
export const fetchAsyncLogin = createAsyncThunk(
  ...
  async (auth: CRED) => {
    const res = await axios.post<JWT>(
      `${ process.env.REACT_APP_API_URL }accounts/jwt/create/`,
      auth,
      {
        headers: {
  ...
  extraReducers: (builder) => {
    builder.addCase(
      ...
      (state, action: PayloadAction<JWT>) => {
        ...
        action.payload.access && (window.location.href = '/articles');
      }
    );
    builder.addCase(
      ...
      (state, action: PayloadAction<PROFILE>) => {
        return {
          ...
          profiles: state.profiles.map(
            (prof) => prof.id === action.payload.id ? action.payload : prof
          ),
        };
      }
    );
